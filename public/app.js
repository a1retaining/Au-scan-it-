...
function renderDetail(s) {
  if (!s) {
    $("detail").innerHTML = `<h3>TRADE DECISION STREAM</h3><p class="muted">Select a signal to inspect entry, stop, target, score breakdown and risk notes.</p>`;
    return;
  }
  ...
  const rr = Number(s.riskReward || 0);
  const score = Number(s.score || 0);
  const gate = score >= 80 && rr >= 1.8 ? "RULES PASS" : "RULES BLOCK";
  ...
  $("detail").innerHTML = `<h3>TRADE DECISION STREAM</h3>
    ...
    <div class="button-row">
      <button class="secondary" onclick="openPaperTrade('${s.symbol}')">Paper Entry</button>
    ...
    <div class="snapshot-list">
      <div class="snapshot-item"><strong>Score rule</strong><span>${score >= 80 ? "PASS" : "BLOCK"} ${score}/80</span></div>
      <div class="snapshot-item"><strong>R:R rule</strong><span>${rr >= 1.8 ? "PASS" : "BLOCK"} ${num(rr)}/1.80</span></div>
    </div>
...
function drawChart(s, incomingBars, sourceLabel) {
  ...
  const bars = (incomingBars || s.bars || []).slice(-120);
  ...
  if ($("chartMeta")) {
    $("chartMeta").textContent = `${shortSymbol(s.symbol)} | ${bars.length} bars | ${currentRange} / ${currentInterval} | Entry, stop, target and paper trades`;
  }
}
...
async function openPaperTrade(symbol) {
  ...
  if (score < 80) {
    toast(`Trade blocked: score must be 80+. Current score ${score}.`);
    return;
  }

  if (rr < 1.8) {
    toast(`Trade blocked: R:R must be at least 1.8. Current R:R ${num(rr)}.`);
    return;
  }
      ...
      <div class="paper-trades">
        ${
          rows
            ...
            .join("") ||
          `<div class="paper-trade"><strong>No paper trades yet</strong><small>Click Paper Entry on a selected ASX signal.</small></div>`
        }
      </div>`;
    }
