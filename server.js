...
const ASX_CONFIG = {
  ...
  open: "10:00",
  close: "16:00",
  dataDelay: "Public/free ASX data is normally delayed. This app does not claim real-time exchange data.",
  yahooSuffix: ".AX"
};
...
const ASX_DISCOVERY_UNIVERSE = uniqueArray([
  ...DEFAULT_ASX_SYMBOLS,
  "A2M.AX", "ABC.AX", "ALD.AX", "ALQ.AX", "ALU.AX", "AMC.AX", "AMP.AX", "APA.AX", "ARB.AX", "ARG.AX",
  "AWC.AX", "AZJ.AX", "BEN.AX", "BOQ.AX", "BPT.AX", "BRG.AX", "BSL.AX", "BWP.AX", "CGF.AX", "CHC.AX",
  "CNU.AX", "DMP.AX", "DOW.AX", "EDV.AX", "EVN.AX", "FLT.AX", "GQG.AX", "HVN.AX", "IEL.AX", "IGO.AX",
  "ILU.AX", "IPL.AX", "JBH.AX", "LLC.AX", "LYC.AX", "MFG.AX", "MGR.AX", "MPL.AX", "NEC.AX", "NXT.AX",
  "ORA.AX", "ORI.AX", "PLS.AX", "PRU.AX", "RHC.AX", "SDF.AX", "SGM.AX", "SGP.AX", "SGR.AX", "SOL.AX",
  "TAH.AX", "TPG.AX", "VCX.AX", "VEA.AX", "VOC.AX", "VUK.AX", "WHC.AX", "WOR.AX", "YAL.AX", "ZIP.AX"
]);
...
function safeDate(timestamp, interval) {
  ...
  return String(interval).includes("m") || String(interval).includes("h") ? d.toISOString() : d.toISOString().slice(0, 10);
}

function cacheTtl(interval, range) {
  if (range === "10y") return 30 * 60 * 1000;
  if (String(interval).includes("m") || String(interval).includes("h")) return 60 * 1000;
  return 4 * 60 * 1000;
}
...
function setCache(key, value) {
  ...
  if (chartCache.size > 1500) chartCache.delete(chartCache.keys().next().value);
}
...
async function fetchText(url, timeoutMs = 16000) {
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: { "User-Agent": "Mozilla/5.0 TradingMintASX/1.0", "Accept": "application/json,text/plain,*/*" }
    });
...
async function yahooChart(symbol, range = "1y", interval = "1d") {
  ...
  let data;
  try { data = JSON.parse(text); } catch (error) { throw new Error("Yahoo returned non JSON for " + cleanSymbol); }
  ...
  if (!quote || !timestamps.length) throw new Error("No Yahoo quote data for " + cleanSymbol);
  const bars = [];
  for (let i = 0; i < timestamps.length; i++) {
    ...
    if (Number.isFinite(open) && Number.isFinite(high) && Number.isFinite(low) && Number.isFinite(close)) {
      bars.push({ date: safeDate(timestamps[i], interval), open: round(open, 4), high: round(high, 4), low: round(low, 4), close: round(close, 4), volume: volume || 0 });
    }
  }
  const minBars = String(interval).includes("m") || String(interval).includes("h") ? 2 : 35;
  if (bars.length < minBars) throw new Error("Yahoo returned too few bars for " + cleanSymbol);
  return bars;
}

async function getBars(symbol, range = "1y", interval = "1d") {
  const cleanSymbol = normalizeAsxSymbol(symbol);
  if (!cleanSymbol.endsWith(".AX")) throw new Error("Only ASX Yahoo symbols are allowed, for example CBA.AX or CBA");
  ...
  const result = { source: "Yahoo Finance public chart endpoint", symbol: cleanSymbol, range, interval, bars, dataReality: ASX_CONFIG.dataDelay };
  setCache(key, result);
  return result;
}
...
function analyzeSymbol(symbol, bars, marketContext = { market: "Unknown" }, source = "unknown") {
  ...
  function add(name, points, reason, warning) {
    ...
    scoreParts.push({ name, points: round(clean, 1) });
  ...
  add("trendStack", ema20 && ema50 && ema200 && ema20 > ema50 && ema50 > ema200 ? 10 : ema20 && ema50 && ema20 > ema50 ? 6 : 0, "Moving averages are aligned bullishly.", "Moving averages are not fully aligned.");
  ...
  add("rsi", rsi14 && rsi14 >= 48 && rsi14 <= 68 ? 10 : rsi14 && rsi14 > 68 && rsi14 <= 76 ? 5 : rsi14 && rsi14 >= 42 && rsi14 < 48 ? 4 : 0, "RSI supports momentum without being too stretched.", "RSI is not in the ideal swing zone.");
  add("volume", volumeRatio && volumeRatio >= 1.5 ? 9 : volumeRatio && volumeRatio >= 1.1 ? 6 : volumeRatio && volumeRatio >= 0.8 ? 3 : 0, "Volume is confirming the move.", "Volume is not confirming the move.");
  add("liquidity", avgDollarVolume20 && avgDollarVolume20 >= 20000000 ? 8 : avgDollarVolume20 && avgDollarVolume20 >= 5000000 ? 4 : 0, "ASX liquidity is acceptable to strong based on 20 day dollar volume.", "Liquidity may be thin for active trading.");
  ...
  let target1 = Math.max(high20 + atr * 0.35, price + atr * 2.0);
  let target2 = Math.max(target1 + atr, price + atr * 3.0);
  if (!Number.isFinite(stop) || stop <= 0 || stop >= price) stop = price - atr * 1.7;
  if (!Number.isFinite(target1) || target1 <= price) target1 = price + atr * 2.0;
  if (!Number.isFinite(target2) || target2 <= target1) target2 = price + atr * 3.0;

  const riskReward = calcRiskReward(price, stop, target1);
  const inBuyZone = price >= buyZoneLow && price <= buyZoneHigh * 1.008;
  const distanceToZone = price > buyZoneHigh ? ((price - buyZoneHigh) / buyZoneHigh) * 100 : price < buyZoneLow ? -((buyZoneLow - price) / buyZoneLow) * 100 : 0;
  let decision = "WATCH";
  let setup = "Watch";
  if (score >= 78 && riskReward >= 1.35 && inBuyZone && price > (ema20 || 0)) { decision = "ENTER_PAPER_ONLY"; setup = "ASX Swing Entry"; }
  else if (score >= 68 && price > (ema20 || 0) && distanceToZone > 1.5) { decision = "WAIT_FOR_PULLBACK"; setup = "ASX Pullback"; }
  else if (score >= 58 && price >= high20 * 0.965) { decision = "BREAKOUT_WATCH"; setup = "ASX Breakout Watch"; }
  ...
  return {
    symbol, market: "ASX", currency: "AUD", decision, setup, score, rankScore: score,
    price: round(price), changePercent: round(changePercent, 2), change5dPct: round(change5dPct, 2), change10dPct: round(change10dPct, 2), change20dPct: round(change20dPct, 2),
    rangePosition20: round(rangePosition20, 1), distanceFromEma20Pct: round(distanceFromEma20Pct, 2),
    buyZoneLow: round(buyZoneLow), buyZoneHigh: round(buyZoneHigh),
    stopLoss: round(stop), target1: round(target1), target2: round(target2), riskReward: round(riskReward, 2),
    support: round(low20), resistance: round(high20), trigger: round(trigger), tightStop: round(tightStop), dayTarget: round(dayTarget),
    dayRiskReward: round(dayRiskReward, 2), distanceToBuyZonePct: round(distanceToZone, 2), avgDollarVolume20: round(avgDollarVolume20, 0),
    summary: decision === "ENTER_PAPER_ONLY" ? symbol + " has a strong ASX swing setup for paper testing only." :
      decision === "WAIT_FOR_PULLBACK" ? symbol + " is strong but should not be chased. Wait for the buy zone." :
      decision === "BREAKOUT_WATCH" ? symbol + " is near a breakout area. Wait for confirmation." : symbol + " is a watchlist candidate only.",
    actionPlan: "ASX share scanner only. Use paper trading first. Options are not fabricated by this app.",
    reasons, warnings, scoreParts, source, dataReality: ASX_CONFIG.dataDelay,
    indicators: { ema5: round(ema5), ema10: round(ema10), ema20: round(ema20), ema50: round(ema50), ema200: round(ema200), rsi14: round(rsi14), atr14: round(atr14), atrPct: round(atrPct, 2), volumeRatio: round(volumeRatio, 2) },
    bars
  };
}
...
function analyzeIntradaySymbol(symbol, bars, source) {
  ...
  const reasons = [];
  const warnings = [];
  if (ema8 && price > ema8) { score += 25; reasons.push("Price is above short EMA."); }
  if (ema8 && ema21 && ema8 > ema21) { score += 20; reasons.push("Short EMA is above medium EMA."); }
  if (rsi14 && rsi14 >= 50 && rsi14 <= 74) { score += 18; reasons.push("RSI supports intraday momentum."); }
  else if (rsi14 && rsi14 > 78) { score += 5; warnings.push("Intraday RSI is hot."); }
  if (price >= recentHigh * 0.995) { score += 20; reasons.push("Price is near intraday breakout area."); }
  if (changePercent && changePercent > 0) { score += 10; reasons.push("Price is moving up this interval."); }
  score = Math.max(0, Math.min(100, Math.round(score)));
  ...
  return { symbol, market: "ASX", currency: "AUD", decision, setup: "ASX Intraday Momentum", score, rankScore: score, price: round(price), changePercent: round(changePercent, 2), trigger: round(trigger), tightStop: round(tightStop), stopLoss: round(tightStop), dayTarget: round(dayTarget), target1: round(dayTarget), riskReward: round(dayRiskReward, 2), dayRiskReward: round(dayRiskReward, 2), summary: decision === "DAY_PAPER_WATCH" ? symbol + " has intraday momentum characteristics for paper testing." : symbol + " is an intraday watch only.", actionPlan: "ASX day paper test only. No averaging down.", reasons, warnings, source, dataReality: ASX_CONFIG.dataDelay, bars };
}
...
async function buildMarketContext() {
  try {
    ...
    const bullish = price > ema20Value && price > ema50Value && price > ema200Value && ema20Value > ema50Value;
    return { market: bullish ? "Bullish" : "Neutral", benchmark: "VAS.AX", source: result.source, benchmarkData: { price: round(price), ema20: round(ema20Value), ema50: round(ema50Value), ema200: round(ema200Value) } };
  } catch (error) {
    return { market: "Unknown", benchmark: "VAS.AX", source: "None", error: error.message };
  }
}
...
async function scanSymbols(symbolList, risk) {
  ...
  const results = await mapLimit(symbolList, 6, async symbol => {
    try {
      ...
      return { ok: true, signal: analyzeSymbol(symbol, result.bars, marketContext, result.source) };
    } catch (error) {
  ...
  const signals = [];
  const errors = [];
  for (const item of results) item && item.ok ? signals.push(item.signal) : errors.push({ symbol: item && item.symbol, error: item && item.error });
  signals.sort((a, b) => Number(b.rankScore || 0) - Number(a.rankScore || 0));
  return { ok: true, version: VERSION, market: "ASX", marketRegime: marketContext.market, config: ASX_CONFIG, risk: Number(risk || 100), requested: symbolList.length, count: signals.length, elapsedMs: Date.now() - startedAt, updatedAt: new Date().toISOString(), signals, errors };
}
...
async function dayScanSymbols(symbolList) {
  ...
  const results = await mapLimit(symbolList, 6, async symbol => {
    try {
      ...
      return { ok: true, signal: analyzeIntradaySymbol(symbol, result.bars, result.source) };
    } catch (error) {
      return { ok: false, symbol, error: error.message };
    }
  });
  ...
  const errors = [];
  for (const item of results) item && item.ok ? signals.push(item.signal) : errors.push({ symbol: item && item.symbol, error: item && item.error });
  signals.sort((a, b) => Number(b.dayRiskReward || 0) * Number(b.score || 0) - Number(a.dayRiskReward || 0) * Number(a.score || 0));
  return { ok: true, version: VERSION, market: "ASX", mode: "asx-day-paper-test", config: ASX_CONFIG, requested: symbolList.length, count: signals.length, elapsedMs: Date.now() - startedAt, updatedAt: new Date().toISOString(), signals, errors };
}
...
function equityStats(equityCurve, trades) {
  ...
  return { startingEquity: round(starting), endingEquity: round(ending), netProfit: round(ending - starting), netReturnPct: starting ? round(((ending - starting) / starting) * 100, 2) : 0, totalTrades: trades.length, wins: wins.length, losses: losses.length, winRate: trades.length ? round((wins.length / trades.length) * 100, 2) : 0, profitFactor: grossLoss > 0 ? round(grossWin / grossLoss, 2) : wins.length ? 99 : 0, maxDrawdownPct: round(maxDrawdownPct, 2) };
}
...
function swingEntryTier(signal) {
  ...
  if (score >= 65 && rr >= 2.2 && near) return "Tier 3";
  return "";
}
...
async function backtestSymbols(symbolList, params) {
  ...
  const results = await mapLimit(symbolList, 4, async symbol => {
    try {
      ...
      return { ok: true, symbol: normalizeAsxSymbol(symbol), source: result.source, bars: result.bars.slice(-Math.floor(params.years * 252)) };
    } catch (error) { return { ok: false, symbol, error: error.message }; }
  });
  for (const r of results) r && r.ok ? loaded.push(r) : errors.push({ symbol: r && r.symbol, error: r && r.error });
  ...
  for (const date of allDates) {
    ...
    for (let i = open.length - 1; i >= 0; i--) {
      ...
      let exitPrice = null;
      let exitReason = "";
      if (bar.low <= pos.stop) { exitPrice = pos.stop; exitReason = "Stop hit"; }
      else if (holdDays >= params.minHoldDays && bar.high >= pos.target) { exitPrice = pos.target; exitReason = "Target hit after minimum hold"; }
      else if (holdDays >= params.maxHoldDays) { exitPrice = bar.close; exitReason = "Max hold reached"; }
      if (exitPrice !== null) {
        const proceeds = exitPrice * pos.shares;
        const pnl = proceeds - pos.cost;
        cash += proceeds;
        trades.push({ symbol: pos.symbol, entryDate: pos.entryDate, exitDate: date, entry: round(pos.entry), exit: round(exitPrice), shares: pos.shares, cost: round(pos.cost), proceeds: round(proceeds), pnl: round(pnl), pnlPct: round((pnl / pos.cost) * 100, 2), holdDays, entryScore: pos.entryScore, riskReward: pos.riskReward, exitReason });
        open.splice(i, 1);
      }
    }
    for (const item of loaded) {
      ...
      if (!swingEntryTier(signal) || signal.decision !== "ENTER_PAPER_ONLY") continue;
      ...
      if (!Number.isFinite(entry) || !Number.isFinite(stop) || !Number.isFinite(target) || rr < 1.3) continue;
      ...
      const shares = Math.max(0, Math.min(Math.floor(params.riskDollars / riskPerShare), Math.floor(maxTradeValue / entry)));
      if (shares < 1) continue;
      const cost = shares * entry;
      cash -= cost;
      open.push({ symbol: item.symbol, entryDate: nextBar.date, entryIndex: row.index + 1, entry, shares, cost, stop, target, lastPrice: entry, entryScore: signal.score, riskReward: round(rr, 2) });
    ...
    equityCurve.push({ date, equity: round(value) });
  }
  ...
  for (let i = open.length - 1; i >= 0; i--) {
    ...
    const pnl = proceeds - pos.cost;
    cash += proceeds;
    trades.push({ symbol: pos.symbol, entryDate: pos.entryDate, exitDate: lastDate, entry: round(pos.entry), exit: round(price), shares: pos.shares, cost: round(pos.cost), proceeds: round(proceeds), pnl: round(pnl), pnlPct: round((pnl / pos.cost) * 100, 2), holdDays: row ? row.index - pos.entryIndex : 0, entryScore: pos.entryScore, riskReward: pos.riskReward, exitReason: "Closed at end of backtest" });
  }
  if (equityCurve.length) equityCurve[equityCurve.length - 1].equity = round(cash);
  return { ok: true, version: VERSION, market: "ASX", mode: "asx-share-backtest", params, requested: symbolList.length, loaded: loaded.length, errors, stats: equityStats(equityCurve, trades), trades: trades.slice(-250).reverse(), equityCurve, updatedAt: new Date().toISOString(), elapsedMs: Date.now() - startedAt, notes: ["Uses public Yahoo Finance ASX chart data only.", "No fake quotes, no fake options, no fake live data.", "Entries are simulated on the next daily open after a qualifying signal.", "Backtesting is historical simulation, not proof of future performance."] };
}
...
function readTrades() {
  ensureTradeFile();
  try { return JSON.parse(fs.readFileSync(tradesPath, "utf8")); } catch (error) { return []; }
}
...
app.get("/api/health", (req, res) => res.json({ ok: true, version: VERSION, service: "TradingMint ASX Real", status: "online", config: ASX_CONFIG, routes: ["/api/scan", "/api/discover", "/api/day-scan", "/api/bars", "/api/backtest", "/api/options", "/api/paper/open", "/api/paper/close", "/api/paper/trades", "/api/paper/stats"], universeCount: ASX_DISCOVERY_UNIVERSE.length, cacheSize: chartCache.size, time: new Date().toISOString() }));
...
app.get("/api/bars", async (req, res) => {
  try {
    ...
    res.json({ ok: true, version: VERSION, market: "ASX", symbol, range, interval, source: result.source, count: result.bars.length, bars: result.bars, dataReality: result.dataReality, updatedAt: new Date().toISOString() });
...
app.get("/api/options", (req, res) => {
  ...
  res.json({
    ...
    message: "This app does not fabricate ASX option chains. Yahoo public chart data provides share bars here, not a reliable live ASX exchange-traded options chain.",
    whatWouldBeNeeded: ["licensed ASX derivatives data", "broker API with ASX options chain", "or a verified options data vendor"],
    scannerBehaviour: "Use share signals now. Only enable options when a real options-chain source is connected."
  });
});
...
app.post("/api/paper/open", async (req, res) => {
  try {
    ...
    const trade = { id: "ASX-" + Date.now(), status: "open", market: "ASX", symbol, side, entry: round(entry, 4), shares, stop: round(req.body.stop), target: round(req.body.target), setup: String(req.body.setup || "manual"), notes: String(req.body.notes || ""), openedAt: new Date().toISOString(), source: "user-entered paper trade" };
    trades.push(trade);
    writeTrades(trades);
    res.json({ ok: true, version: VERSION, trade });
...
app.get("/api/paper/trades", (req, res) => res.json({ ok: true, version: VERSION, trades: readTrades().slice().reverse() }));

app.get("/api/paper/stats", (req, res) => {
  const closed = readTrades().filter(t => t.status === "closed");
  const wins = closed.filter(t => Number(t.pnl) > 0);
  const losses = closed.filter(t => Number(t.pnl) < 0);
  const bySetup = {};
  for (const t of closed) {
    const key = t.setup || "unknown";
    bySetup[key] = bySetup[key] || { trades: 0, wins: 0, pnl: 0 };
    bySetup[key].trades += 1;
    if (Number(t.pnl) > 0) bySetup[key].wins += 1;
    bySetup[key].pnl += Number(t.pnl || 0);
  }
  for (const key of Object.keys(bySetup)) bySetup[key].winRate = round((bySetup[key].wins / bySetup[key].trades) * 100, 2);
  res.json({ ok: true, version: VERSION, stats: { closedTrades: closed.length, wins: wins.length, losses: losses.length, winRate: closed.length ? round((wins.length / closed.length) * 100, 2) : 0, netPnl: round(closed.reduce((s, t) => s + Number(t.pnl || 0), 0), 2), bySetup } });
});

app.get("/api/sectors", (req, res) => res.json({ ok: true, market: "ASX", sectors: ASX_SECTORS }));
app.get("/api/universe", (req, res) => res.json({ ok: true, market: "ASX", count: ASX_DISCOVERY_UNIVERSE.length, symbols: ASX_DISCOVERY_UNIVERSE }));

app.use(express.static(publicPath));
app.use((req, res) => {
  if (req.path.startsWith("/api/")) return res.status(404).json({ ok: false, version: VERSION, error: "API route not found", path: req.path });
